...
import com.zhexuan.divination.service.ZiweiChartService;
...
public class ZiweiChartServiceImpl implements ZiweiChartService {
    ...
    public Map<String, Object> buildChart(ZiweiChartDTO dto) {
        ...
        String flowYearStem = STEMS[Math.floorMod(currentYear - 4, 10)];
        String flowYearBranch = BRANCHES[Math.floorMod(currentYear - 4, 12)];
        Map<String, String> flowYearTransformations = fourHua(flowYearStem);
        Map<String, Object> flowYearInfo = buildFlowYearInfo(palaces, flowYearStem, flowYearBranch, flowYearTransformations);
        ...
        result.put("flowYear", flowYearInfo);
    ...
    private Map<String, Object> buildFlowYearInfo(List<Map<String, Object>> palaces, String flowYearStem, String flowYearBranch, Map<String, String> flowYearTransformations) {
        Map<String, Object> flowYear = new LinkedHashMap<String, Object>();
        flowYear.put("year", LocalDate.now().getYear());
        flowYear.put("ganZhi", flowYearStem + flowYearBranch);
        flowYear.put("stem", flowYearStem);
        flowYear.put("branch", flowYearBranch);
        flowYear.put("transformations", flowYearTransformations);

        // 流年命宫位置：流年地支对应的宫位
        int flowYearMingIndex = palaceBranchIndex(flowYearBranch);
        flowYear.put("mingPalaceIndex", flowYearMingIndex);
        flowYear.put("mingPalaceName", PALACE_NAMES[0]);

        // 流年十二宫：从流年命宫开始，依次排列
        List<Map<String, Object>> flowYearPalaces = new ArrayList<Map<String, Object>>();
        for (int i = 0; i < 12; i++) {
            int palaceIndex = Math.floorMod(flowYearMingIndex + i, 12);
            Map<String, Object> basePalace = palaces.get(palaceIndex);
            Map<String, Object> fyPalace = new LinkedHashMap<String, Object>();
            fyPalace.put("branch", basePalace.get("branch"));
            fyPalace.put("stem", basePalace.get("stem"));
            fyPalace.put("ganZhi", basePalace.get("ganZhi"));
            fyPalace.put("name", PALACE_NAMES[i]);
            fyPalace.put("isFlowYearMing", i == 0);
            fyPalace.put("basePalaceName", basePalace.get("name"));
            fyPalace.put("mainStars", basePalace.get("mainStars"));
            fyPalace.put("luckyStars", basePalace.get("luckyStars"));
            fyPalace.put("shaStars", basePalace.get("shaStars"));
            fyPalace.put("otherStars", basePalace.get("otherStars"));
            flowYearPalaces.add(fyPalace);
        }
        flowYear.put("palaces", flowYearPalaces);

        // 流年四化落在哪些宫位
        Map<String, String> flowYearHuaPositions = new LinkedHashMap<String, String>();
        for (Map.Entry<String, String> entry : flowYearTransformations.entrySet()) {
            String star = entry.getKey();
            String hua = entry.getValue();
            for (Map<String, Object> palace : palaces) {
                ...
                if (stars.contains(star)) {
                    flowYearHuaPositions.put(star + hua, palace.get("name") + "(" + palace.get("branch") + ")");
                }
            }
        }
        flowYear.put("huaPositions", flowYearHuaPositions);
