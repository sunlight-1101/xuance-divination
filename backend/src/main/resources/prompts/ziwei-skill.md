...
7. 使用知识库案例时，只能类比，不得替代用户原盘。必须先确认案例与用户盘面在关键宫位、主星/四化、问题主题上有明显相同点；若不同点较多，要写明”只能作为参考倾向”并降低确定性。
8. 引用典籍和规则时只引用能直接支持当前判断的内容，不要引用目录、泛泛介绍或与本盘无关的章节。若引用案例，必须在 `keyEvidence` 或 `detailedAnalysis` 中写清”相同点 + 差异点 + 可借鉴结论”。

流年分析要求：
- 系统已提供 [FLOW_YEAR] 段落，包含流年干支、流年命宫、流年四化、流年四化落宫、流年十二宫。
- 必须结合流年信息分析当年运势，说明流年四化对本命盘的影响。
- 流年四化落在哪个宫位，就分析该宫位代表的领域在当年的变化。
- 如果用户问的是近期运势或流年趋势，流年分析必须作为重点。
...
- 若使用了案例规则，`keyEvidence` 中至少一条要说明该案例为什么可类比；若没有足够相同点，不要引用该案例。
