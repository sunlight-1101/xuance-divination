...
OUTPUT = Path("pet-runs/luckclub_classic_import.sql")
REPORT = Path("pet-runs/luckclub_classic_import_report.txt")
...
BOOKS = [
    BookSpec("渊海子平", "渊海子平", "yuan-hai-zi-ping", "八字子平基础", "宋", "徐升", 50),
    BookSpec("子平真诠", "子平真诠", "zi-ping-zhen-quan", "八字格局用神", "清", "沈孝瞻", 60),
    BookSpec("神峰通考", "神峰通考", "shen-feng-tong-kao", "八字病药中和", "明", "张楠", 70),
    BookSpec("千里命稿", "千里命稿", "qian-li-ming-gao", "现代八字应用", "民国", "韦千里", 90),
    BookSpec("命理探原", "命理探原", "ming-li-tan-yuan", "命理源流与五行神煞", "民国", "袁树珊", 92),
    BookSpec("滴天髓阐微", "滴天髓阐微", "di-tian-sui-chan-wei", "八字体用清浊阐释", "清", "任铁樵", 21),
]
...
def book_header(spec: BookSpec, path: Path, pages: int) -> list[str]:
    ...
    source_note = "PDF 内含可抽取文字，已按页切入典籍库；文本含原文与白话译文，适合检索参考。"
    return [
    ...
    for spec in BOOKS:
        ...
        for page_number, page in enumerate(reader.pages, start=1):
            ...
            for part_index, chunk in enumerate(chunk_text(text), start=1):
                ...
                lines.append(chapter_sql(title, f"PDF第{page_number}页", page_number * 10 + part_index, chunk, "luckclub 可抽取文本。"))
                inserted += 1
