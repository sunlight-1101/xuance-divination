...
OUTPUT = Path("pet-runs/luckclub_liuyao_import.sql")
REPORT = Path("pet-runs/luckclub_liuyao_import_report.txt")
...
BOOKS = [
    BookSpec("卜筮正宗", "卜筮正宗", "bu-shi-zheng-zong", "六爻体系与取用", "清", "王洪绪", 121),
    BookSpec("卜筮全书", "卜筮全书", "bu-shi-quan-shu", "六爻纳甲总集", "明", "", 122),
    BookSpec("增删卜易", "增删卜易", "zeng-shan-bu-yi-pdf", "六爻断法案例", "清", "野鹤老人", 130),
    BookSpec("断易天机", "断易天机", "duan-yi-tian-ji", "六爻分类断法", "", "", 135),
    BookSpec("黄金策", "黄金策", "huang-jin-ce", "火珠林断法纲领", "明", "刘基", 136),
]
...
def book_header(spec: BookSpec, path: Path, pages: int) -> list[str]:
    ...
    source_note = "PDF 内含可抽取文字，已按页切入典籍库；文本含原文与白话译文，适合六爻检索参考。"
    return [
...
def chapter_sql(title: str, volume: str, order: int, text: str) -> str:
    return (
        ...
        f"(@book_id, {sql(title)}, {sql(volume)}, {sql(order)}, {sql(text)}, {sql(text)}, {sql('luckclub 可抽取文本。')}, NULL, 'FULL', NOW(), NOW());"
    )
    ...
    for spec in BOOKS:
        ...
        for page_number, page in enumerate(reader.pages, start=1):
            ...
            for part_index, chunk in enumerate(chunk_text(text), start=1):
                ...
                lines.append(chapter_sql(f"第{page_number}页{suffix}", f"PDF第{page_number}页", page_number * 10 + part_index, chunk))
                inserted += 1
        report.append(f"{spec.name}: pages={len(reader.pages)}, chapters={inserted}, file={path.name}")
